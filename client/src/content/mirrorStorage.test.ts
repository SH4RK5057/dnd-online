...
function emptyContent(): MirrorContent {
  return { spells: [], monsters: [], items: [], importedAt: 0, sourceKey: '' }
}
...
describe('ingestFile category filtering', () => {
  ...
  it('skips excluded categories silently (no error, nothing ingested)', () => {
    const content = emptyContent()
    const errors: string[] = []
    const monstersOnly = { includeSpells: false, includeMonsters: true, includeItems: false }
  ...
  it('imports only items when spells/monsters are excluded', () => {
    const content = emptyContent()
    const errors: string[] = []
    const itemsOnly = { includeSpells: false, includeMonsters: false, includeItems: true }
  ...
  it('still reports genuinely unrecognized file shapes as errors, category filtering aside', () => {
    ...
    expect(errors).toEqual(['junk.json: no recognized "spell"/"monster"/"item" array — expected 5etools-2014-src shape'])
  })
